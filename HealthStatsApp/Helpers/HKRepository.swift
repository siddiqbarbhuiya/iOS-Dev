...
final class HKRepository {
    ...
    func requestHealthStat(by category: String, completion: @escaping ([HealthStat]) -> Void) {
        guard let store = store, let type = HKObjectType.quantityType(forIdentifier: typeCategory(category: category)) else {
            return
        }
    ...
    private func typeCategory(category: String) -> HKQuantityTypeIdentifier {
        switch category {
